...
const Categories = ({ category, theme }) => {
  ...
  // Function to get the product status based on stock
      function getProductStatus(stock) {
        ...
        return ""; // No label if stock is more than 10
      }

      // Sample rendering of products
      items.forEach(product => {
        ...
        console.log(`
          <div class="product-card">
            ...
            ${productStatus === "Sold Out" ? `<div class="status sold-out">${productStatus}</div>` : ""}
            ${productStatus === "Fast Selling" ? `<div class="status fast-selling">${productStatus}</div>` : ""}
            <h3>${product.name}</h3>
            <p>₱${product.price.toFixed(2)} PHP</p>
          </div>
        `);
      });
